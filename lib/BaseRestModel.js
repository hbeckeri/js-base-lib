...
module.exports = (Model = BaseModel) => {
	return class BaseRestModel extends BasePostgresModel(BaseRouter(Model)) {
		...
		static get restRouter() {
			...
			this.rpcMethods.map(each => {
				router.get(`/${each.name}`, this.middlewares.rpc, this[each.command].bind(this));
			});
			...
			Object.keys(this.Postgres.associations).forEach(key => {
				...
				if (a.associationType === 'BelongsToMany') {
					router.get(
						`/:id/${a.as.toLowerCase()}`,
						this.middlewares.read,
						async (req, res, next) => {
							try {
								const instance = await this.Postgres.findByPk(req.params.id);
		...
		static async list(req, res, next) {
			try {
				...
				const order = sort
					? [[sort.startsWith('-') ? sort.substr(1) : sort, sort.startsWith('-') ? 'DESC' : 'ASC']]
					: undefined;
